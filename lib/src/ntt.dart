...
List<int> ntt(ParameterSet parameters, List<int> w) {
  final List<int> wHat =
      List<int>.generate(256, (int i) => w[i], growable: false);

  int m = 0;
  final int q = parameters.q();

  for (int len = 128; len >= 1; len = (len / 2).floor()) {
...
List<int> nttInverse(ParameterSet parameters, List<int> wHat) {
  final List<int> w =
      List<int>.generate(256, (int i) => wHat[i], growable: false);
...
List<int> addNtt(ParameterSet parameters, aHat, List<int> bHat) {
  return List<int>.generate(
      256, (int i) => modQ(aHat[i] + bHat[i], parameters.q()));
}
...
List<int> multiplyNtt(ParameterSet parameters, aHat, List<int> bHat) {
  return List<int>.generate(
      256, (int i) => modMultiply(aHat[i], bHat[i], parameters.q()));
}

List<List<int>> vectorNtt(ParameterSet parameters, List<List<int>> vHat) {
  return List<List<int>>.generate(
      vHat.length, (int i) => ntt(parameters, vHat[i]));
}

List<List<int>> vectorNttInverse(
    ParameterSet parameters, List<List<int>> vHat) {
  return List<List<int>>.generate(
      vHat.length, (int i) => nttInverse(parameters, vHat[i]));
}

List<List<int>> subtractVectorNtt(
    ParameterSet parameters, List<List<int>> vHat, List<List<int>> wHat) {
  return List<List<int>>.generate(
      vHat.length, (int i) => subtractNtt(parameters, vHat[i], wHat[i]));
}

List<List<int>> scalarVectorNtt(
    ParameterSet parameters, List<int> cHat, List<List<int>> vHat) {
  return List<List<int>>.generate(
      vHat.length, (int i) => multiplyNtt(parameters, cHat, vHat[i]));
}

// ignore: non_constant_identifier_names
List<List<int>> matrixVectorNtt(
    ParameterSet parameters, List<List<List<int>>> MHat, List<List<int>> vHat) {
  final List<List<int>> wHat = List<List<int>>.generate(
    parameters.k(),
    (int _) {
      return List<int>.generate(parameters.l(), (int _) => 0);
    },
    growable: false,
  );

  for (int i = 0; i < parameters.k(); i++) {
    for (int j = 0; j < parameters.l(); i++) {
      wHat[i] = addNtt(
          parameters, wHat[i], multiplyNtt(parameters, MHat[i][j], vHat[j]));
    }
  }
