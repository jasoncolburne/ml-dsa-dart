...
Uint8List bitsToBytes(List<int> y) {
    ...
    final List<int> z = List<int>.generate(((alpha+7)/8).floor(), (int _) => 0, growable: false);
...
List<int> bytesToBits(Uint8List z) {
	...
	final List<int> zPrime = List<int>.generate(zLength, (int i) => z[i], growable: false);
	final List<int> y = List<int>.generate(8*zLength, (int _) => 0, growable: false);
...
(Uint8List, Uint8List, Uint8List, List<List<int>>, List<List<int>>, List<List<int>>)
...
Uint8List sigEncode(ParameterSet parameters, Uint8List cTilde, List<List<int>> z, List<List<int>> h) {
	final List<int> sigma = List<int>.generate(cTilde.length, (int i) => cTilde[i]);
...
(Uint8List, List<List<int>>, List<List<int>>?)
sigDecode(ParameterSet parameters, Uint8List sigma) {
	final int width = 32 * (1 + (parameters.gamma1()-1).bitLength);

	final Uint8List cTilde = sigma.sublist(0, (parameters.lambda()/4).floor());
	final Uint8List x = sigma.sublist((parameters.lambda()/4).floor(), (parameters.lambda()/4).floor() + parameters.l() * width);
    final Uint8List y = sigma.sublist((parameters.lambda()/4).floor() + parameters.l() * width);

	final z = List<List<int>>.generate(parameters.l(), (int i) {
		final int offset = i * width;
		final int limit = offset + width;
		return bitUnpack(x.sublist(offset, limit), parameters.gamma1()-1, parameters.gamma1());
	}, growable: false);
...
Uint8List w1Encode(ParameterSet parameters, List<List<int>> w) {
	...
	for (int i = 0; i < w.length; i++) {
		final int b = ((parameters.q()-1)/(2*parameters.gamma1())).floor() - 1;
...
Uint8List hintBitPack(ParameterSet parameters, List<List<int>> h) {
	final List<int> y = List<int>.generate(parameters.omega()+parameters.k(), (int _) => 0, growable: false);
	int index = 0;
